...
public class Express {
	...
	static int doroga = 76;   // код дороги, к которой относятся данные
   ...
   // кол-во видов перевозок (для перебора в цикле)
	...
	// readFile(boolean oneFio, String nameFileRead, String view_tr, String view_pass, int numRow)
		readFile(true, "BSLP.txt", "по решению руководства OAO PЖД", "иные лица", 13);
		readFile(true, "BSLSP.txt", "по решению руководства OAO PЖД", "иные лица", 13);
		readFile(true, "DSLP.txt", "по договорам с OAO PЖД", "иные лица", 20);
		readFile(true, "DSLSP.txt", "по договорам с OAO PЖД", "иные лица", 20);
		readFile(true, "FSLP.txt", "", "работники организаций, финансируемых ОАО РЖД", 20);
		readFile(true, "FSLSP.txt", "", "работники организаций, финансируемых ОАО РЖД", 20);
		readFile(false, "ILNP.txt", "по личным надобностям", "дети, находящиеся на иждевении работников и пенсионеров ОАО РЖД", 21);
		readFile(false, "ILNSP.txt", "по личным надобностям", "дети, находящиеся на иждевении работников и пенсионеров ОАО РЖД", 21);
		readFile(true, "NLNP.txt", "по личным надобностям", "дети, родители которых погибли в результате ж.д. аварий", 13);
		readFile(true, "NLNSP.txt", "по личным надобностям", "дети, родители которых погибли в результате ж.д. аварий", 13);
		readFile(true, "PLNP.txt", "", "пенсионеры", 16);
		readFile(true, "PLNSP.txt", "", "пенсионеры", 16); 
		readFile(true, "RLNP.txt", "по личным надобностям", "работники ОАО РЖД", 15);
		readFile(true, "RLNSP.txt", "по личным надобностям", "работники ОАО РЖД", 15);
		readFile(true, "RRUP.txt", "от дома до работы/учебы", "работники ОАО РЖД", 15);
		readFile(true, "RRUSP.txt", "от дома до работы/учебы", "работники ОАО РЖД", 15);
		readFile(true, "RSNP.txt", "служебные", "работники ОАО РЖД", 15);
		readFile(true, "RSNSP.txt", "служебные", "работники ОАО РЖД", 15);
